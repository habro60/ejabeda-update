...
if (!empty($_POST['acc_code'])) {
    ...
    if (!empty($amount['acc_type']) && !empty($amount['gl_acc_code'])) {
        ...
        $sql1 = "SELECT invoice_detail.order_no FROM `invoice_detail` WHERE invoice_detail.gl_acc_code ='$gl_code' AND `bill_status`='SV' AND `item_status`='1' GROUP BY invoice_detail.order_no";
...
if (!empty($_POST['order_no_cal'])) {
    ...
    // total amount
    $sql2 = "SELECT SUM(total_price_loc) as balance2 FROM `invoice_detail` WHERE order_no='$order_no_cal' AND bill_status='SV'";
