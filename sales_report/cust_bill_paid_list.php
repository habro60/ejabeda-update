...
if (empty($cust_id)) {
    $sql2 = "SELECT  cust_info.id, cust_info.cust_name, cust_info.gl_acc_code, sales_detail.gl_acc_code FROM  cust_info, sales_detail where cust_info.gl_acc_code = sales_detail.gl_acc_code ORDER BY cust_info.gl_acc_code, sales_detail.order_date";
} else {
    $sql2 =  "SELECT  cust_info.id, cust_info.cust_name, cust_info.gl_acc_code, sales_detail.gl_acc_code FROM  cust_info, sales_detail where cust_info.gl_acc_code = $cust_id ORDER BY cust_info.gl_acc_code, sales_detail.order_date";
}
...
<main class="app-content">
    ...
    <div class="app-title">
        <div>
            <h1><i class="fa fa-dashboard"></i>Customer Bill Paid Report</h1>
        </div>
    </div>
    <!-- end page title -->
    <div class="row">
        ...
        <table id="submit">
            <form method="POST">
                ...
                <option value="">--- Select Customer---</option>
                    <?php
                    $selectQuery = "SELECT cust_info.id, cust_info.cust_name, cust_info.gl_acc_code FROM `cust_info`";
                    $selectQueryResult =  $conn->query($selectQuery);
                    if ($selectQueryResult->num_rows) {
                        while ($glrow = $selectQueryResult->fetch_assoc()) {
                            echo '<option value="' . $glrow['gl_acc_code'] . '">'  . $glrow['cust_name'] . '</option>';
                                }
                            }
                            ?>
                          </select></td>
    ...
    <?php
        ...
        <table id="submit">
            ...
            ?>
                ...
                <h4 style="text-align:center" id="reportTitle">Customer Bill Paid Report</h4>
                ...
                <?php
                        ...
                        $no = 1;
                        $batch_no = 0;
                        $total_sales_amt =0.00;
                        $tot_paid_amt=0.00;
                        $tot_dr_amt =0.00;
                        $tot_cr_amt =0.00;
                        $total_sales_item_amt=0.00;
                ...
                if (isset($_POST['submit'])) {
                    ...
                    if (empty($office_code)) {
                        if (empty($gl_acc_code)) {
                            $sql="select tran_details.tran_date,tran_details.batch_no, tran_details.gl_acc_code, tran_details.vaucher_type, tran_details.dr_amt_loc, tran_details.cr_amt_loc, sales_detail.order_no,sales_detail.gl_acc_code,sales_detail.item_no,sales_detail.item_qty,sales_detail.unit_price_loc,sales_detail.total_price_loc, sales_detail.item_unit, sales_detail.include_discount_rate, sales_detail.include_discount_amt, sales_detail.include_vat_rate, sales_detail.include_vat_amt, sales_detail.include_tax_rate, sales_detail.include_tax_amt, gl_acc_code.acc_head as cust_name,item.id,item_name, code_master.description from tran_details,sales_detail,item, gl_acc_code, code_master where tran_details.batch_no=sales_detail.order_no and tran_details.gl_acc_code=sales_detail.gl_acc_code and tran_details.tran_mode=sales_detail.order_type and (sales_detail.bill_status='CHQR' or sales_detail.bill_status='CASR') and (sales_detail.order_type='CHQR' or sales_detail.order_type='CASR') and gl_acc_code.acc_code=tran_details.gl_acc_code and sales_detail.item_no=item.id AND (tran_details.tran_date BETWEEN '$startdate' AND '$enddate') and code_master.hardcode='UCODE' and sales_detail.item_unit=code_master.softcode order by tran_details.tran_date, tran_details.batch_no";

                        }else {
                            $gl_acc_code = $conn->escape_string($_POST['gl_acc_code']);
                            $sql="select tran_details.tran_date,tran_details.batch_no, tran_details.gl_acc_code, tran_details.vaucher_type, tran_details.dr_amt_loc, tran_details.cr_amt_loc, sales_detail.order_no,sales_detail.gl_acc_code,sales_detail.item_no,sales_detail.item_qty,sales_detail.unit_price_loc,sales_detail.total_price_loc, sales_detail.item_unit, sales_detail.include_discount_rate, sales_detail.include_discount_amt, sales_detail.include_vat_rate, sales_detail.include_vat_amt, sales_detail.include_tax_rate, sales_detail.include_tax_amt, gl_acc_code.acc_head as cust_name,item.id,item_name, code_master.description from tran_details,sales_detail,item, gl_acc_code, code_master where tran_details.batch_no=sales_detail.order_no and tran_details.gl_acc_code=sales_detail.gl_acc_code and tran_details.tran_mode=sales_detail.order_type and (sales_detail.bill_status='CHQR' or sales_detail.bill_status='CASR') and (sales_detail.order_type='CHQR' or sales_detail.order_type='CASR')  and gl_acc_code.acc_code=tran_details.gl_acc_code and sales_detail.item_no=item.id AND (tran_details.tran_date BETWEEN '$startdate' AND '$enddate') and tran_details.gl_acc_code ='$gl_acc_code' and code_master.hardcode='UCODE' and sales_detail.item_unit=code_master.softcode order by tran_details.tran_date, tran_details.batch_no";

                        }
                        
                    } else {   
                        if (empty($gl_acc_code)) { 
                            $sql="select tran_details.tran_date,tran_details.batch_no, tran_details.gl_acc_code, tran_details.vaucher_type, tran_details.dr_amt_loc, tran_details.cr_amt_loc, sales_detail.order_no,sales_detail.gl_acc_code,sales_detail.item_no,sales_detail.item_qty,sales_detail.unit_price_loc,sales_detail.total_price_loc, sales_detail.item_unit, sales_detail.include_discount_rate, sales_detail.include_discount_amt, sales_detail.include_vat_rate, sales_detail.include_vat_amt, sales_detail.include_tax_rate, sales_detail.include_tax_amt, gl_acc_code.acc_head as cust_name,item.id,item_name, code_master.description from tran_details,sales_detail,item, gl_acc_code, code_master where tran_details.batch_no=sales_detail.order_no and tran_details.gl_acc_code=sales_detail.gl_acc_code and tran_details.tran_mode=sales_detail.order_type and (sales_detail.bill_status='CHQR' or sales_detail.bill_status='CASR') and (sales_detail.order_type='CHQR' or sales_detail.order_type='CASR')  and gl_acc_code.acc_code=tran_details.gl_acc_code and sales_detail.item_no=item.id AND (tran_details.tran_date BETWEEN '$startdate' AND '$enddate') and tran_details.office_code='$office_code'  and  code_master.hardcode='UCODE' and sales_detail.item_unit=code_master.softcode order by tran_details.tran_date, tran_details.batch_no";

                        } else {
                            $gl_acc_code = $conn->escape_string($_POST['gl_acc_code']);
                            $sql="select tran_details.tran_date,tran_details.batch_no, tran_details.gl_acc_code, tran_details.vaucher_type, tran_details.dr_amt_loc, tran_details.cr_amt_loc, sales_detail.order_no,sales_detail.gl_acc_code,sales_detail.item_no,sales_detail.item_qty,sales_detail.unit_price_loc,sales_detail.total_price_loc, sales_detail.item_unit, sales_detail.include_discount_rate, sales_detail.include_discount_amt, sales_detail.include_vat_rate, sales_detail.include_vat_amt, sales_detail.include_tax_rate, sales_detail.include_tax_amt, gl_acc_code.acc_head as cust_name,item.id,item_name, code_master.description from tran_details,sales_detail,item, gl_acc_code, code_master where tran_details.batch_no=sales_detail.order_no and tran_details.gl_acc_code=sales_detail.gl_acc_code and tran_details.tran_mode=sales_detail.order_type and (sales_detail.bill_status='CHQR' or sales_detail.bill_status='CASR') and (sales_detail.order_type='CHQR' or sales_detail.order_type='CASR') and gl_acc_code.acc_code=tran_details.gl_acc_code and sales_detail.item_no=item.id AND (tran_details.tran_date BETWEEN '$startdate' AND '$enddate') and tran_details.office_code='$office_code'  and tran_detail.gl_acc_code= '$gl_acc_code' and code_master.hardcode='UCODE' and sales_detail.item_unit=code_master.softcode order by tran_details.tran_date, tran_details.batch_no";
                        }
                    ...
                    <td style="text-align: right"><?php if ($rows['batch_no'] != $batch_no) {
                      ...
                      <tfoot>
                        <tr style="background-color:powderblue";>
                                ...
                                <th style="text-align:right" colspan="1"><?php echo $total_sales_item_amt; ?></th>
