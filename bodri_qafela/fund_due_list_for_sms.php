...
<main class="app-content">
                    ...
                    <td>Member: <select class="form-control grand" name="member_id">
                            ...
                            if ($selectQueryResult->num_rows) {
                                while ($drrow = $selectQueryResult->fetch_assoc()) {
                                    echo '<option value="' . $drrow['gl_acc_code'] . '">'  . $drrow['full_name'] . '</option>';
                                }
                            }
                            ?>
                          </select></td>
    ...
    <?php
        ...
        <table id="submit">
            ...
            <form name="sms" action="SMS_fund_due.php" method="post" target="_blank">
                <!-- report header -->
                ...
                <div class="pull-right">
                    <button type="submit" id="submit" name="submit" class="btn btn-danger Submit" target="_blank"></i>Send SMS</button>
                </div>
                ...
                if (isset($_POST['submit'])) {
                    ...
                    $enddate = $conn->escape_string($_POST['enddate']);
                    $gl_acc_code = $conn->escape_string($_POST['member_id']);

                    // echo $member_id;
                    // die;
                    if (empty($member_id)) {
                            $sql="SELECT donner_fund_detail.fund_type, donner_fund_detail.member_id,donner_fund_detail.fund_type_desc,donner_fund_detail.num_of_pay,donner_fund_detail.donner_pay_amt,donner_fund_detail.num_of_paid,donner_fund_detail.due_amt,donner_fund_detail.waiver_amt,donner_fund_detail.donner_paid_amt, fund_member.member_id,fund_member.gl_acc_code, fund_member.full_name FROM `donner_fund_detail`,fund_member where fund_member.member_id=donner_fund_detail.member_id and donner_fund_detail.donner_pay_amt > '0' order by donner_fund_detail.member_id,donner_fund_detail.fund_type"; 
                ...
                ?>
                <tr>
                    <td><input type="checkbox" name="member_id_cheack[]" value=<?php echo $rows['member_id'];  ?> class="select_one"></td>
                    ...
                    <td style="text-align: left"><?php if ($rows['member_id'] != $mem_id) {
                        ...
                        <td style="background-color:powderblue; text-align: right"><?php $tot_pay_amt = $tot_pay_amt + $rows['donner_pay_amt']; echo ($rows['donner_pay_amt'] * $rows['num_of_pay']);?></td>                                                                 
                        ...
                        <td style="background-color:lightgray; text-align: right"><?php $tot_paid_amt = $tot_paid_amt + $rows['donner_paid_amt']; echo $rows['donner_paid_amt']; ?></td>   
                          ...
                          <td style="background-color:powderblue; text-align: right"><?php echo $rows['due_amt'];?></td> 
                          
                            <td style="background-color:powderblue; text-align: right"><?php  echo $rows['waiver_amt'];?></td>    
                          
                          
                        <td style="background-color:white; text-align: right"><?php $due_num_of_pay = $rows['num_of_pay'] - $rows['num_of_paid']; $due_amt = (($rows['donner_pay_amt'] * $due_num_of_pay) + $rows['due_amt'] - $rows['waiver_amt']);  echo $due_amt; ?></td>                                                            
                </tr>
                        <?php
                            }
                        }
