...
if (isset($_POST["submit"])) {
    ...
    // echo $insertQuerydr; exit;
    $conn->query($insertQuerydr);
    if ($conn->affected_rows == 1) {
        $massage_dr = "dr Save Successfully!!";
    }
    $insertQuerydis = "INSERT INTO `tran_details` (`tran_no`, `auto_tran_no`, `office_code`, `year_code`, `batch_no`, `tran_date`, `back_value_date`, `gl_acc_code`, `tran_mode`, `vaucher_type`, `dr_amt_loc`, `cr_amt_loc`, `dr_amt_fc`, `cr_amt_fc`, `bank_name`, `branch_name`, `cheque_no`, `cheque_date`, `curr_code`, `exch_rate`, `description`, `reversaled`, `rev_tran_no`, `auto_tran`, `posted`, `verified_by_1`, `verified_date_1`, `pay_on_vouch_no`, `purchase_date`, `bill_paid_flag`, `ss_creator`, `ss_creator_on`, `ss_modified`, `ss_modified_on`, `ss_org_no`) VALUES (NULL, '0', '$office_code', '$year_code', '$batch_no', '$tran_date', '9999-09-09 18:50:44', '$discount_id', '$tran_mode_discount', '$vaucher_typedr', '$discount_amount', '0.00', '0.00', '0.00', '', '', '', '9999-09-09', '', '0.0000', '$discount', '', '', '', '', '', '9999-09-09 00:00:00', '0', '$tran_date', '1', '$ss_creator', now(), 'N/A', now(), '$ss_org_no')";
    if ($discount_amount != 0) {
        $conn->query($insertQuerydis);
    } else {
        echo  $conn->error;
    }
    $insertQueryvat = "INSERT INTO `tran_details` (`tran_no`, `auto_tran_no`, `office_code`, `year_code`, `batch_no`, `tran_date`, `back_value_date`, `gl_acc_code`, `tran_mode`, `vaucher_type`, `dr_amt_loc`, `cr_amt_loc`, `dr_amt_fc`, `cr_amt_fc`, `bank_name`, `branch_name`, `cheque_no`, `cheque_date`, `curr_code`, `exch_rate`, `description`, `reversaled`, `rev_tran_no`, `auto_tran`, `posted`, `verified_by_1`, `verified_date_1`, `pay_on_vouch_no`, `purchase_date`, `bill_paid_flag`, `ss_creator`, `ss_creator_on`, `ss_modified`, `ss_modified_on`, `ss_org_no`) VALUES (NULL, '0', '$office_code', '$year_code', '$batch_no', '$tran_date', '9999-09-09 18:50:44', '$vat_id', '$tran_mode_vat', '$vaucher_typecr', '0.00', '$vat_amount', '0.00', '0.00', '', '', '', '9999-09-09', '', '0.0000', '$vat', '', '', '', '', '', '9999-09-09 00:00:00', '0', '$tran_date', '1', '$ss_creator', now(), 'N/A', now(), '$ss_org_no')";
    if ($vat_amount != 0) {
        $conn->query($insertQueryvat);
    } else {
        echo  $conn->error;
    }
    $insertQuerytax = "INSERT INTO `tran_details` (`tran_no`, `auto_tran_no`, `office_code`, `year_code`, `batch_no`, `tran_date`, `back_value_date`, `gl_acc_code`, `tran_mode`, `vaucher_type`, `dr_amt_loc`, `cr_amt_loc`, `dr_amt_fc`, `cr_amt_fc`, `bank_name`, `branch_name`, `cheque_no`, `cheque_date`, `curr_code`, `exch_rate`, `description`, `reversaled`, `rev_tran_no`, `auto_tran`, `posted`, `verified_by_1`, `verified_date_1`, `pay_on_vouch_no`, `purchase_date`, `bill_paid_flag`, `ss_creator`, `ss_creator_on`, `ss_modified`, `ss_modified_on`, `ss_org_no`) VALUES (NULL, '0', '$office_code', '$year_code', '$batch_no', '$tran_date', '9999-09-09 18:50:44', '$tax_id', '$tran_mode_tax', '$vaucher_typecr', '0.00', '$tax_amount', '0.00', '0.00', '', '', '', '9999-09-09', '', '0.0000', '$tax', '', '', '', '', '', '9999-09-09 00:00:00', '0', '$tran_date', '1', '$ss_creator', now(), 'N/A', now(), '$ss_org_no')";
    if ($tax_amount != 0) {
    ...
    for ($count = 0; $count < count($_POST["item_no"]); $count++) {
        ...
        $for_gl_acc_code   = $_POST["to_account"];
        $tran_date  = $tran_date;
    ...
    <form action="" method="post">
        ...
        <table class="table bg-light table-bordered table-sm">
            <tfoot>
                <tr>
                    <td></td>
                    <td><select class="form-control" name="to_account" required>
                            <option value="">--Select Sales A/C---</option>

                            <?php
                            $selectQuery = "SELECT * FROM `gl_acc_code` where postable_acc= 'Y' and category_code = '1'  and acc_type='20' ORDER by acc_code";
                    ...
                    <td><input type="text" class="cr_amount form-control" id="cr_amount" name="craccount" placeholder="Cr.Amount" readonly></td>
                ...
                <tr>
                    <td></td>
                    <td><select class="form-control grand" id="by_account" style="width:250px" onchange="byAccount()" name="by_account" required>
                            <option value="">--Sales By Cash/Chq/Credit A/c--</option>
                            <?php
                    ...
                    <td colspan="2"> <input type="text" class="form-control" name="particulardr" placeholder="Particular" id="particulardr"></td>
                    <td style="width: 250px"> <input type="text" id="dr_amount" class="dr_amount form-control" name="draccount" placeholder="Dr. Amount" readonly></td>
                    <td style="width: 20px"> <strong>Dr</strong></td>
                </tr>
                <tr id="cheque_no_td" hidden>
                    <td></td>
                    <td>
                        <input class="form-control" name="cheque_no" id="cheque_no" placeholder="Cheque No.">
                    </td>
            ...
            function discount(count) {
                ...
                if (discount > 0) {
                    ...
                    if (discount > 0) {
                        ...
                        $('#dr_amount').val(discount_after_amount);
                    } else {
                        $('#grand_total').val(sub_total);
                        $('#dr_amount').val(sub_total);
                    }
            ...
            function vat(count) {
                ...
                if (vat > 0) {
                    ...
                    if (vat_after_amount > 0) {
                        ...
                        $('#dr_amount').val(vat_after_amount);
                    } else {
                        vat_before_amount = $('#sub_total').val();
                        $('#grand_total').val(vat_before_amount);
                        $('#dr_amount').val(vat_before_amount);
                    }
                } else {}
            }
            ...
            function tax(count) {
                ...
                if (tax > 0) {
                    ...
                    if (tax_after_amount > 0) {
                        ...
                        $('#dr_amount').val(tax_after_amount);
                    } else {
                        ttt = $('#sub_total').val();
                        discountamount = $('#discount_amount').val();
                        discounvat = $('#vat_amount').val();
