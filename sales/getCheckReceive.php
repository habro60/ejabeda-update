...
if (!empty($_POST['acc_code'])) {
    ...
    $sql = "SELECT tran_details.gl_acc_code,gl_acc_code.acc_type, SUM(tran_details.dr_amt_loc - tran_details.cr_amt_loc) as balance FROM `tran_details` JOIN `gl_acc_code` where tran_details.gl_acc_code=`gl_acc_code`.`acc_code` AND tran_details.gl_acc_code =$gl_acc_code AND tran_details.tran_mode='SV'";
    $payAmount = mysqli_query($conn, $sql);
    $amount = mysqli_fetch_assoc($payAmount);
...
if (!empty($_POST['gl_acc_code'])) {
    ...
    // $sql = "SELECT tran_details.gl_acc_code, gl_acc_code.acc_type,tran_details.tran_mode,tran_details.batch_no, SUM(tran_details.cr_amt_loc) as balance FROM `tran_details` JOIN `gl_acc_code` where tran_details.gl_acc_code=`gl_acc_code`.`acc_code` AND tran_details.tran_mode='PV' AND tran_details.batch_no =$gl_acc_code";
    $sql = "SELECT SUM(total_price_loc) as balance FROM `invoice_detail` WHERE order_no='$gl_acc_code' AND bill_status='SV'";
