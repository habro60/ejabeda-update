...
if (isset($_POST["submit"])) {
    ...
    // cr
    $insertQuerycr = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`, `description`, `cr_amt_loc`,`cheque_no`,`cheque_date`,`pay_on_vouch_no`,`purchase_date`,`bill_paid_flag`,`ss_creator`,`ss_creator_on`,`ss_modified_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$to_account','$tran_mode','$vaucher_typecr','$particularcr','$craccount','$cheque_no','$cheque_date','$pay_on_vouch_no','$purchase_date','$bill_paid_flag','$ss_creator',now(),now(),'$ss_org_no')";
    $conn->query($insertQuerycr);
    if ($conn->affected_rows == 1) {
        $massage_cr = "cr Save Successfully!!";
    }
    $insertQuerydis = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`,`description`, `cr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_modified_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$discount_id','$tran_mode_discount','$vaucher_typecr','$discount','$discount_amount','$ss_creator',now(),now(),'$ss_org_no')";
    if ($discount_amount != 0) {
        $conn->query($insertQuerydis);
    } else {
        echo  $conn->error;
    }
    $insertQueryvat = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`,`description`, `dr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_modified_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$vat_id','$tran_mode_vat','$vaucher_typedr','$vat','$vat_amount','$ss_creator',now(),now(),'$ss_org_no')";
    if ($vat_amount != 0) {
    ...
    echo "<meta http-equiv=\"refresh\" content=\"0;URL=purchase_voucher.php\">";
}
            ...
            <tbody>
                <tr>
                    ...
                    <td><select name="item_no[]" class="form-control item_no unitChange" required>
                            ...
                            <?php
                            $selectQuery = "SELECT id, item_name FROM `item` where  (sellable_option='N' or sellable_option='B')";
            ...
            <tfoot>
                <tr>
                    ...
                    <td><select class="form-control select2" name="by_account" style="width:250px" required>
                            ...
                            <?php
                            $selectQuery = "SELECT * FROM `gl_acc_code` where  (category_code = '4' or category_code = '1') and acc_type > '2' and postable_acc= 'Y' ORDER by acc_code";
                            $selectQueryResult =  $conn->query($selectQuery);
                            if ($selectQueryResult->num_rows) {
                ...
                <tr>
                    ...
                    <td><select class="form-control grand" name="to_account" onchange="toAccount()" id="to_account" required>
                            ...
                            <?php
                            $selectQuery = "SELECT * FROM `gl_acc_code` where  acc_type='1' OR acc_type='2' OR acc_type='6' and postable_acc= 'Y' ORDER BY acc_code";
                            $selectQueryResult =  $conn->query($selectQuery);
                            if ($selectQueryResult->num_rows) {
                                while ($drrow = $selectQueryResult->fetch_assoc()) {
                                    echo '<option value="' . $drrow['acc_code'] . '">'  . $drrow['acc_head'] . '</option>';
