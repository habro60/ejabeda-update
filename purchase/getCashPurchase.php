...
// end unit code 
if (!empty($_POST['proj_phase_no'])) {
    $id = $_POST['proj_phase_no'];
    $selectQuery = "SELECT DISTINCT indent_no, indent_from FROM `indent_info` WHERE indent_from ='$id'";
    $result = mysqli_query($conn, $selectQuery);
    ?>
    ...
    ?>
        <option value="<?php echo $data["indent_no"]; ?>"><?php echo $data["indent_no"]; ?></option>
<?php
    };
}
?>
...
?>
    ...
    ?>
        <tr>
            ...
            <!-- delivery quantity -->
            <td><input type="text" name="item_qty[]" id="item_qty1<?php echo $data['item_no']; ?>" data-srno="1" class="item_qty form-control" value="" onkeyup="unitPrice('<?php echo $data['item_no']; ?>')" /></td>
            <!-- ! -->
...
if (!empty($_POST['item_no_deli'])) {
    ...
    $selectQuery = "SELECT *, SUM(CASE WHEN in_out_flag = 1 THEN item_qty ELSE -item_qty END) AS current_stock
        FROM invoice_detail
        WHERE item_no = '$id'";
    //  $selectQuery = "SELECT * FROM invoice_detail WHERE order_type='PVC' AND item_no='$id'";
    $result = mysqli_query($conn, $selectQuery);
    $data = mysqli_fetch_assoc($result);
  // Assuming $data contains the two values you want to return
$response = array(
    'unit_price_loc' => $data['unit_price_loc'],
