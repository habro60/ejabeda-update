...
<main class="app-content">
  ...
  <table>
    <form method="POST">
      <tr>
        <td>
          <input type="number" name="order_no" id="" class="form-control" placeholder="Input Voucher No" required>
        </td>
        <td>
          <input type="submit" name="submit" value="submit" class="form-control btn-info">
        </td>
      </tr>
    </form>
  </table>
  ...
  ?>
    <table class="table table-hover table-bordered table-sm">
      ...
      <tbody>
        <?php
        $sql = "SELECT invoice_detail.id, invoice_detail.item_no, invoice_detail.order_date,invoice_detail.order_no, invoice_detail.item_unit, invoice_detail.item_qty AS item_qty,invoice_detail.unit_price_loc as unit_price_loc, invoice_detail.total_price_loc as total_price_loc, invoice_detail.item_status, item.item_name as item_name FROM invoice_detail JOIN item WHERE invoice_detail.item_no=item.id AND invoice_detail.order_no='$order_no' AND (invoice_detail.order_type='PV' or invoice_detail.order_type='PVC' OR invoice_detail.order_type='PVCHQ')";
  ...
  ?>
    <table class="table table-hover table-bordered table-sm">
      ...
      <tbody>
        <?php
        $sql = "SELECT invoice_detail.id, invoice_detail.item_no, invoice_detail.order_date,invoice_detail.order_no, invoice_detail.item_unit, invoice_detail.item_qty AS item_qty,invoice_detail.unit_price_loc as unit_price_loc, invoice_detail.total_price_loc as total_price_loc, invoice_detail.bill_status, invoice_detail.item_status, item.item_name as item_name FROM invoice_detail JOIN item WHERE invoice_detail.item_no=item.id AND (invoice_detail.order_type='PV' or invoice_detail.order_type='PVC' OR invoice_detail.order_type='PVCHQ')";
        $query = $conn->query($sql);
        while ($row = $query->fetch_assoc()) {
          echo '<tr>';
          echo '<td>' . $row['id'] . '</td>';
