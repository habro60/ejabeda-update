    ...
    ?>
      <table class="table table-hover table-bordered">
        ...
        <tbody>
          <?php
          //  $q = intval($_GET['q']);
          $sql = "SELECT apart_owner_info.owner_id, apart_owner_info.office_code, apart_owner_info.owner_name, apart_owner_info.owner_image,office_info.office_name FROM `apart_owner_info`, `office_info` WHERE apart_owner_info.office_code = office_info.office_code";
          $query = $conn->query($sql);
          while ($row = $query->fetch_assoc()) {
