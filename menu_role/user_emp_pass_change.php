...
if (isset($_POST['update'])) {
  ...
  if ($password == $confirm_password) {
    ...
    if ($conn->affected_rows == 1) {
      ...
      $update = "UPDATE sm_hr_emp SET username='$username' WHERE emp_no='$id'";
      $conn->query($update);
      header("refresh:1;user_create.php");
    } else {
      $user_create_failled = "Failled!!";
      header("refresh:1;user_password_change.php?id=" . $_GET['id'] . "&&user_group=" . $_GET['user_group'] . "&&name=" . $_GET['name'] . "");
    }
  } else {
    $confirm = "Failled!!";
    header("refresh:1;user_password_change.php?id=" . $_GET['id'] . "&&user_group=" . $_GET['user_group'] . "&&name=" . $_GET['name'] . "");
