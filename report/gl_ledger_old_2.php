...
<main class="app-content">
    ...
    <div id="organizationAdd">
             ...
             <div>
                ...
                <div id="reporttitle">
                     <h6 style="text-align:center" id="reportTitle"><b>
                         <?php echo $ownerRows['acc_head']; ?></b></h6> 
                </div>
                 
                <div id="reporttitle">
                     <h6 style="text-align:center" id="reportTitle"><b>Report Title :  </b>Ledger Account</h6> 
                </div>
                <div id="AsOnDate">
            ...
            <div id="mySelector">
                <table style="width:850px;margin-top:50px;margin-left:120px;margin-right:80px;border: 1px solid black;border-collapse: collapse;" id="sampleTable">
                    <thead class="reportHead">
                        ...
                        <th style="text-align:center;border: 1px solid black;">Vch Type</th>
                        <th style="text-align:center;border: 1px solid black;">Vch No</th>
                        <th style="text-align:center;border: 1px solid black;">Particular</th>
                        <th style="text-align:center;border: 1px solid black;">Debit In TK</th>
                        <th style="text-align:center;border: 1px solid black;">Credit In Tk</th>
                        <th style="width:100px;text-align:center;border: 1px solid black;">Balance In TK</th>
                    </thead>
                    <tbody style="">
                        <?php
                        $sql_opbal = "SELECT SUM(`dr_amt_loc`) as `debit`, SUM(`cr_amt_loc`) AS `credit` FROM tran_details WHERE gl_acc_code='$account' AND (`tran_date` BETWEEN '$org_fin_year_st' AND '$open_bal') ORDER BY `tran_date`";
                        ...
                        <tr style="font-weight:bold;color: red;text-align: center;">
                            <td style="border: 1px solid black;" colspan="5">Opening Balance </td>
                        ...
                        if (($category_code =='2') ||($category_code =='3')) {
                            $sql="SELECT tran_details.tran_no,tran_details.gl_acc_code,tran_details.batch_no,tran_details.tran_mode,tran_details.dr_amt_loc,tran_details.tran_date,tran_details.tran_mode,tran_details.description,tran_details.cr_amt_loc,gl_acc_code.acc_head,gl_acc_code.acc_code,tran_details.cr_amt_loc-tran_details.dr_amt_loc as balance, @RunningBalance:= @RunningBalance + (tran_details.cr_amt_loc-tran_details.dr_amt_loc) as RunningBalance, gl_acc_code.category_code  FROM tran_details JOIN gl_acc_code JOIN (SELECT @RunningBalance:= 0) r WHERE tran_details.gl_acc_code='$account' and (gl_acc_code.category_code='2' or gl_acc_code.category_code='3') and (tran_details.tran_date BETWEEN '$startdate' AND '$enddate')group by tran_details.tran_no order by tran_details.tran_date, tran_details.tran_no";
                        } 
                        $category_code= $ownerRows['category_code'];
                        if (($category_code =='1') ||($category_code =='4')) {
                            
                            $sql="SELECT tran_details.tran_no,tran_details.gl_acc_code,tran_details.batch_no,tran_details.tran_mode,tran_details.dr_amt_loc,tran_details.tran_date,tran_details.tran_mode,tran_details.description,tran_details.cr_amt_loc,gl_acc_code.acc_head,gl_acc_code.acc_code,tran_details.dr_amt_loc-tran_details.cr_amt_loc as balance, @RunningBalance:= @RunningBalance + (tran_details.dr_amt_loc-tran_details.cr_amt_loc) as RunningBalance, gl_acc_code.category_code  FROM tran_details JOIN gl_acc_code JOIN (SELECT @RunningBalance:= 0) r WHERE tran_details.gl_acc_code='$account' and tran_details.gl_acc_code=gl_acc_code.acc_code and  (gl_acc_code.category_code='1' or gl_acc_code.category_code='4') and (tran_details.tran_date BETWEEN '$startdate' AND '$enddate') group by tran_details.tran_no  order by tran_details.tran_date, tran_details.tran_no" 
                            ;         
                            }
                        $i=1;
                        ...
                        while ($rows = $query->fetch_assoc()) {
                            if ($rows['tran_date'] >= $startdate) {
                                ...
                                <td style="text-align:center;width:15%;border: 1px solid black;border-collapse: collapse;padding:10px;">' . date('d-m-Y ',strtotime($rows['tran_date'])) . '</td>
                                <td style="text-align:center;width:5%;border: 1px solid black;border-collapse: collapse;padding:10px;">' . $rows['tran_mode'] . '</td>
                                <td style="text-align:center;width:5%;border: 1px solid black;border-collapse: collapse;padding:10px;">' . $rows['batch_no'] . '</td>
                                <td style="text-align:center;width:45%;border: 1px solid black;border-collapse: collapse;padding:10px;">' . $rows['description'] . '</td>
      ...
      <div style='padding:80px'>
        <div style='float:left;text-align:left;line-height:100%'><label>--------------------</label><br><?php echo $q; ?>Created by</div>
    
        
        <div style='float:right;text-align:right;line-height:100%'><label>--------------------------</label><br><?php echo $b;?>Authorized by</div></div>
                </div>
    </div>
