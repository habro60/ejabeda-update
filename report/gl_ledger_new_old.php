    ...
    <div id="organizationAdd">
             ...
             <div>
                 ...
                 $sql="SELECT tran_details.tran_no,tran_details.gl_acc_code,tran_details.tran_mode,tran_details.dr_amt_loc,tran_details.tran_date,tran_details.tran_mode,tran_details.description,tran_details.cr_amt_loc,gl_acc_code.acc_head,gl_acc_code.acc_code,tran_details.cr_amt_loc-tran_details.dr_amt_loc as balance, @RunningBalance:= @RunningBalance + (tran_details.cr_amt_loc-tran_details.dr_amt_loc) as RunningBalance, gl_acc_code.category_code  FROM tran_details JOIN gl_acc_code JOIN (SELECT @RunningBalance:= 0) r WHERE tran_details.gl_acc_code='$account' and (gl_acc_code.category_code='2' or gl_acc_code.category_code='3') and (tran_details.tran_date BETWEEN '$startdate' AND '$enddate')group by tran_details.tran_no order by tran_details.tran_date, tran_details.tran_no";
                  ...
                  $return_opbal = mysqli_query($conn, $sql);
                        $result_opbal = mysqli_fetch_assoc($return_opbal);
                        
                        
                         $gl_acc_name="SELECT gl_acc_code.id,gl_acc_code.acc_head, gl_acc_code.category_code from gl_acc_code where  gl_acc_code.acc_code='$account'";
                            $gl_acc_name_run = mysqli_query($conn, $gl_acc_name);
                            $account_name = mysqli_fetch_assoc($gl_acc_name_run);
                            $account_name_row= $account_name['acc_head'];
                ...
                <div id="reporttitle">
                     <h6 style="text-align:center" id="reportTitle">General Ledger Report</h6> 
                </div>
                <div id="reporttitle">
                     <h6 style="text-align:center" id="reportTitle"><?php echo $account_name_row; ?></h6> 
                </div>
                <div id="AsOnDate">
                    <p  style="text-align:center; font-weight:bold"><?php echo "From:"; echo date('d-m-Y ',strtotime($startdate)); ?> To <?php echo date('d-m-Y ',strtotime($enddate)); ?>
            ...
            <table style="margin:20px; border: 1px solid #808080;border-collapse: collapse;margin-top:30px; text-align:center;" id="sampleTable">
                        ...
                        <th style="text-align:center; border: 1px solid #808080;border-collapse: collapse;padding:2px">SL NO</th>
                        <th style="text-align:center;width:10%;border: 1px solid #808080;border-collapse: collapse;padding:2px;">Date</th>
                        <th style="text-align:center;width:5%;border: 1px solid #808080;border-collapse: collapse;padding:2px;">Trx. Mode</th>
                        <th style="text-align:center;width:10%;border: 1px solid #808080;border-collapse: collapse;padding:2px;">Particular</th>
                        <th style="text-align:right;width:10%;border: 1px solid #808080;border-collapse: collapse;padding:2px;">Debit</th>
                        <th style="text-align:right;width:10%;border: 1px solid #808080;border-collapse: collapse;padding:2px;">Credit</th>
                        <th style="text-align:right;width:10%;border: 1px solid #808080;border-collapse: collapse;padding:2px;">Balance</th>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        while ($rows = $query->fetch_assoc()) {
                            if ($rows['tran_date'] >= $startdate) {
                                ...
                                <td style="text-align:left;width:10%;border: 1px solid black;border-collapse: collapse;padding:20px;">' . date('d-m-Y ',strtotime($rows['tran_date'])) . '</td>
                                <td style="text-align:left;width:5%;border: 1px solid black;border-collapse: collapse;padding:10px;">' . $rows['tran_mode'] . '</td>
                                <td style="text-align:left;width:30%;border: 1px solid black;border-collapse: collapse;padding:10px;">' . $rows['description'] . '</td>
                        ...
                        <tr style="font-weight:bold;border: 1px solid black;">
                            ...
                            <td style="border: 1px solid black" colspan="1">Sub Total</td>
                    ...
                    <tfoot>
                        ...
                        <tr class="text-center">
                            <td style="border: 1px solid black;" colspan="3"></td>
                            <td style="border: 1px solid black;" colspan="0"><strong>Total</strong></td>
                            <td style="border: 1px solid black;padding:10px;" colspan="1" ><strong><?php echo number_format($result['debit'], 2, '.', ','); ?></strong>
                            </td>
                            <td style="border: 1px solid black;padding:10px;"><strong><?php echo number_format($result['credit'], 2, '.', ','); ?></strong>
                            </td>
                            <td style="border: 1px solid black;padding:10px;"><strong><?php
                            
                            $val=$result['debit'] - $result['credit'];
                            echo number_format($val, 2, '.', ',');   ?></strong>
                            </td>
                        </tr>
                    <?php
                } else {
