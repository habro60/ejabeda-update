...
if (isset($_POST['submit'])) {
  ...
  for ($count = 0; $count < count($_POST['id']); ++$count) {
    ...
    // process date ..
    $bill_date = $_POST['bill_date'][$count];
    // payment date .. 
    $bill_last_pay_date = $_POST['bill_last_pay_date'][$count];
    // process date ..
    $bill_process_date =  $_POST['bill_date'][$count];
    // --------------------------

    $bill_charge_code = $_POST['bill_charge_code'][$count];
    $bill_charge_name = $_POST['bill_charge_name'][$count];
    $bill_pay_method = $_POST['bill_pay_method'][$count];
    $pay_curr = 'BDT';
    ...
    $ss_created_on = $_POST['bill_date'][$count];
    $ss_org_no = $_SESSION['org_no'];
    if ($role_no > 0) {
      $query = "INSERT INTO `apart_owner_bill_detail` (`id`,`office_code`,`owner_id`,`bill_for_month`,`bill_date`,`bill_charge_code`,`bill_charge_name`,`bill_last_pay_date`,`bill_paid_flag`,`bill_paid_date`,pay_method, pay_curr, bill_process_date,`bill_amount`,`owner_gl_code`, flat_no,`link_gl_acc_code`,batch_no,regular_pay_flag,`ss_creator`,`ss_created_on`,`ss_org_no`) values (NULL, '$office_code','$owner_id','$bill_for_month','$bill_date','$bill_charge_code','$bill_charge_name','$bill_last_pay_date','0','0000-00-00', '$bill_pay_method', '$pay_curr','$bill_process_date','$bill_fixed_amt','$gl_acc_code','$flat_no','$link_gl_acc_code','$lastRows','$regular_pay_flag','$ss_creator','$ss_created_on','$ss_org_no')";
  ...
  header('refresh:1;owner_mth_bill_process.php');
   echo "<meta http-equiv=\"refresh\" content=\"0;URL=owner_mth_bill_process.php\">";
}
