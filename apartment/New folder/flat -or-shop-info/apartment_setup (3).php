...
function billamount($bill_pay_method,$bill_fixed_amt,$flat_area){
  ...
  // require "../database.php";
  // $sql="SELECT * FROM `apart_bill_charge_setup` WHERE bill_charge_code='$bill_charge_code'";
  // $selectQueryResult = $conn->query($sql);
  // $row = $selectQueryResult->fetch_assoc();
  // if($row['bill_pay_method']=='SFT'){
...
//echo $owner_id;
  ...
  //update owner id and flat no
        $updateQuery = "UPDATE `flat_info` SET owner_id='$owner_id',billing_gl_code='$acc_code', flat_status='1', `status_date`=now(), `ss_modifier`='abdullah',`ss_modified_on`=now() WHERE id='$id'";
...
// echo $owner_id;
// echo '***';
// update gl account and billing aaccount
...
//insert gl code
 ...
 (`id`,`office_code`, `acc_code`, `acc_head`, `postable_acc`,`rep_glcode`,`is_ho_acc`,
 ...
 VALUES (NULL,'$office_code','$acc_code',concat('$flat_no', '$acc_head'),'$postable_acc',
 ...
 '$subsidiary_group_code','$ss_creator',now(),now(),'$ss_org_no')";
  ...
  for ($count = 0; $count < count($_POST['id']); ++$count) {
    ...
    if ($owner_id > 0) {
      $query = "update `apart_owner_sevice_facility` set `owner_id`='$owner_id' , `allow_flag`='$allow_flag', `bill_fixed_amt`='$bill_fixed_amt',`effect_date`='$effect_date',`terminate_date`='$terminate_date', ss_creator='$ss_creator', ss_creator_on='$ss_creator_on', ss_org_no='$ss_org_no' where id='$id'";
      $conn->query($query);
    }
...
$sql= "INSERT INTO `monthly_bill_entry`(`owner_id`,`flat_no`,`bill_for_month`,`bill_charge_code`, `bill_charge_name`,`bill_pay_method`) VALUES ('$owner_id','$flat_no','0000-00-00','6','Electric Bill','variable')";
  // $sql= "INSERT INTO `monthly_bill_entry`(`owner_id`,`flat_no`,`bill_for_month`) VALUES ('$owner_id,'$flat_no',NULL)";
        ...
        <label class="col-2 col-form-label">Flat No </label>
         ...
         <input type="hidden" name="flat_name" id="flat_name" value="<?php echo $row['flat_title'] ?>">
          ...
          <div class="col-8 mb-3">
              ...
              <select <?php if($old_owner_id){ echo "disabled";} ?> style="text-align:left;" id="owner_id" name="owner_id" class="form-control select2">
