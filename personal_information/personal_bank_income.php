...
if (isset($_POST["submit"])) {
    ...
    for ($count = 0; $count < count($_POST["particular"]); $count++) {
        ...
        echo "<meta http-equiv=\"refresh\" content=\"0;URL=personal_bank_income.php\">";
}
...
<main class="app-content">
    <form action="" method="post">
        <table class="table bg-light table-bordered table-sm">
            <thead>
                <td>
                    <th>Credit Voucher to Bank A/C</th>
            ...
            <tbody>
                <tr>
                    ...
                    <td><select class="form-control grand" name="to_account[]" calculateToAccount()" id="to_account" required>
                                ...
                                <?php
                                $owner_rec_id = $_SESSION['link_id'];
                                $selectQuery = "SELECT * from personal_account where `owner_rec_id`=$owner_rec_id and (`category_code`='1' or category_code='3') order by id";
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#add_row', function() {
                ...
                html_code += `<td> <select class="form-control select2" name="by_account[]" onchange="by_account()" id="to_account"  required>
                                <option value="">---Select---</option>
                                <?php
                                $selectQuery = "SELECT * from personal_account where `owner_rec_id`=$owner_rec_id and `category_code`='2'  order by id";
                                $selectQueryResult =  $conn->query($selectQuery);
