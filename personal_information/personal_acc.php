...
if (isset($_POST['submit'])) {
  ...
  $insertData = "INSERT INTO personal_account (office_code, owner_rec_id, acc_head,curr_code,category_code,rec_status,active,ss_creator, ss_created_on,ss_modifier,ss_modified_on,ss_org_no) values('$office_code','$owner_rec_id', '$acc_head', 'BDT','$category_code','1','1','$ss_creator','$ss_created_on','$ss_creator','$ss_created_on',
    '$ss_org_no')";
      ...
      <div class="tile-body">
        ...
        <table class="table table-hover table-bordered" id="memberTable">
          ...
          <tbody>
            ...
            $no = 1;
            $owner_rec_id = $_SESSION['link_id'];
            $sql = "SELECT id, office_code, owner_rec_id, acc_head,category_code  from personal_account where owner_rec_id= $owner_rec_id";
             //use for MySQLi-OOP
            ...
            while ($row = $query->fetch_assoc()) {
                ...
                ?>
              <td> <?php if ($row['category_code'] =='1'){echo "Cash";} elseif ($row['category_code'] =='2'){echo "Bank";}elseif ($row['category_code'] =='3'){echo "Income";}elseif ($row['category_code'] =='4'){echo "Expense";}?> </td>
             <td><a  <?php  echo "href='personal_acc_edit.php?id=" . $row['id'] . "'" ?> class='btn btn-success btn-sm'><span class='fa fa-plus'></span>Edit</a>
                      </td>
            <?php  
            }
            ?>
