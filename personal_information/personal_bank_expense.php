...
if (isset($_POST["submit"])) {
    ...
    for ($count = 0; $count < count($_POST["particular"]); $count++) {
            ...
            $vaucher_type = "DR";
            $dr_tran_amt= $_POST['tran_amt'][$count];
            $cr_tran_amt= '0';
            $tran_mode = $tran_mode='CHQP';;  
        ...
        //   $sql = "SELECT * FROM `personal_account` where `owner_rec_id`=$owner_rec_id and category_code='2' ORDER by id";
        ...
        $conquery = "INSERT INTO personal_ledger (office_code,owner_rec_id,`tran_date`,`gl_acc_code`,`tran_mode`,`vaucher_type`,`dr_amt_loc`,`cr_amt_loc`,`dr_amt_fc`,`cr_amt_fc`,description,`bank_name`,`branch_name`,`cheque_no`,`cheque_date`,curr_code,exch_rate,posted,verified_by_1,verified_date_1,ss_creator,ss_creator_on,ss_org_no) VALUES ('$office_code', '$owner_rec_id','$tran_date','$to_account','$tran_mode','$vaucher_type','$cr_tran_amt','$dr_tran_amt','0','0', '$particular','null','null','null','9999-09-09','BDT','0','1','1',now(),'$ss_creator',now(),'$ss_org_no')";
        // echo $conquery;','$
        // exit;
        $conn->query($conquery);
        ...
        echo "<meta http-equiv=\"refresh\" content=\"0;URL=personal_bank_expense.php\">";
}
...
<main class="app-content">
    <form action="" method="post">
        <table class="table bg-light table-bordered table-sm">
            <thead>
                <td>
                    <th>Debit Voucher from Bank A/C</th>
        ...
        <table class="table bg-light table-bordered table-sm" id="multi_table">
            ...
            <tbody>
                <tr>
                    <td>1</td>
                    <td><select class="form-control grand" name="by_account[]" calculateToAccount()" id="by_account" required>
                    
                                <option value="">---Select---</option>
                                
                                <?php
                                $owner_rec_id = $_SESSION['link_id'];
                                $selectQuery = "SELECT * from personal_account where `owner_rec_id`=$owner_rec_id and  category_code='4' order by id";
                    ...
                    <td><select class="form-control grand" name="to_account[]" calculateToAccount()" id="to_account" required>
                                ...
                                <?php
                                $owner_rec_id = $_SESSION['link_id'];
                                $selectQuery = "SELECT * from personal_account where `owner_rec_id`=$owner_rec_id and  category_code='2' order by id";
                                $selectQueryResult =  $conn->query($selectQuery);
                                if ($selectQueryResult->num_rows) {
                                    while ($drrow = $selectQueryResult->fetch_assoc()) {
                                        echo '<option value="' . $drrow['id'] . '">'  . $drrow['acc_head'] . '</option>';
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#add_row', function() {
                ...
                html_code += `<td> <select class="form-control select2" name="by_account[]" onchange="by_account()" id="by_account"  required>
                                <option value="">---Select---</option>
                                <?php
                                $selectQuery = "SELECT * from personal_account where `owner_rec_id`=$owner_rec_id and  category_code='4' order by id";
                                $selectQueryResult =  $conn->query($selectQuery);
