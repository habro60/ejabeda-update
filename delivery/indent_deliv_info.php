...
if (isset($_POST["submit"])) {
    ...
    for ($count = 0; $count < count($_POST['item_no']); ++$count) {
        ...
        if ($item_no > 0) {
            $updateQuery = "UPDATE `indent_info` SET   `delivery_qty`=($due_qty + $delivery_qty), `due_qty`=($due_qty - $deli_qty),  ss_modifier='$ss_modifier', `ss_modified_on`=now(),`ss_org_no`='$ss_org_no', `indent_status`='1' where `indent_no`='$indent_no' and item_no='$item_no'";
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select name="indent_from" class="form-control indent_from" onchange="getIndent(this.value)" required>
                    ...
                    <?php
                    $selectQuery = "SELECT DISTINCT indent_info.indent_from, office_info.office_name, office_info.office_code FROM indent_info, office_info where indent_status='0' AND  indent_info.indent_from=office_info.office_code";
