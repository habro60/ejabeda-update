...
<?php
$output = '';
if (!empty($_POST['cust_order_no'])) {
    $id = $_POST['cust_order_no'];
    $selectQuery = "SELECT DISTINCT cust_order_info.cust_order_no, cust_order_info.order_from, cust_order_info.item_no, cust_order_info.item_qty, cust_order_info.unit_price, cust_order_info.exp_tot_amt_loc,cust_order_info.delivery_qty, cust_order_info.due_qty, item.item_name, item.unit, code_master.hardcode, code_master.softcode, code_master.description FROM `cust_order_info`, item, code_master WHERE code_master.hardcode='UCODE' AND code_master.softcode =item.unit and cust_order_info.cust_order_no ='$id' AND cust_order_info.due_qty!='0' and cust_order_info.item_no=item.id";
    $result = mysqli_query($conn, $selectQuery);
?>
    <?php
    while ($data = mysqli_fetch_assoc($result)) {
