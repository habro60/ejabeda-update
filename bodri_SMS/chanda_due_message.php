            ...
            <form method="POST">
                <tr>
                    <td>Owner : <select class="form-control grand" name="member_id">
                          ...
                          <td> As on Date: <input type="date" name="enddate" id="" class="form-control">
                    ...
                    if (empty($member_id)) {
                     ...
                     + (MONTH('$enddate') 
                         - MONTH(last_paid_date)))  as due_month, sum(mthly_rate_setup.rate_amt) as pay_amt from donner_fund_detail,fund_member, mthly_rate_setup where (mthly_rate_setup.rate_date > donner_fund_detail.last_paid_date and mthly_rate_setup.rate_date  <= '$enddate') and donner_fund_detail.member_id=fund_member.member_id and allow_flag >'0' and donner_fund_detail.last_paid_date <= '$enddate' group by fund_member.member_no order by fund_member.member_no"; 
                            ...
                            - YEAR(last_paid_date)) 
                     + (MONTH('$enddate') 
                         - MONTH(last_paid_date)))  as due_month, sum(mthly_rate_setup.rate_amt) as pay_amt from donner_fund_detail,fund_member, mthly_rate_setup where (mthly_rate_setup.rate_date > donner_fund_detail.last_paid_date and mthly_rate_setup.rate_date  <= '$enddate') and donner_fund_detail.member_id=fund_member.member_id and allow_flag >'0' and donner_fund_detail.last_paid_date <= '$enddate' AND donner_fund_detail.member_id=$member_id group by fund_member.member_no order by fund_member.member_no";
