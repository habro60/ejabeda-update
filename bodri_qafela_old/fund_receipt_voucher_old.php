...
if (isset($_POST['subBtn'])) {
    ...
    $insertQuery = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`, `description`, `cr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$to_account','$tran_mode','$vaucher_typecr','$particular','$craccount','$ss_creator',now(),'$ss_org_no')";
    ...
    for ($i = 0; $i < count($_POST['flag']); $i++) {
        ...
        if ($flag == 1) {
            ...
            $insertFund = "INSERT INTO `fund_receive_detail`  (`tran_no`,batch_no,`office_code`,`member_id`, gl_acc_code,paid_mode, `paid_amount`, `paid_date`,`fund_type`,`fund_type_desc`, `ss_creator`,`ss_creator_on`,`ss_modified_on`,`ss_org_no`) VALUES (NULL,'$batch_no','$office_code','$member_id', '$to_account','CR','$donner_pay_amt', now(),'$fund_type','$fund_type_desc','$ss_creator',now(),now(),'$ss_org_no')";
