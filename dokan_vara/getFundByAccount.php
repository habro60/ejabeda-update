...
if (!empty($_POST['gl_acc_code'])) {
    ...
    // $sql = "SELECT SUM(total_price_loc) as balance FROM `invoice_detail` WHERE order_no='$gl_acc_code' AND bill_status='PV'";
    $sql = "SELECT batch_no, tran_date, gl_acc_code, tran_mode, vaucher_type, dr_amt_loc, cr_amt_loc as balance, pay_on_vouch_no, bill_paid_flag FROM `tran_details` WHERE batch_no ='$gl_acc_code' AND tran_mode='PV' AND vaucher_type='CR' AND pay_on_vouch_no='' AND bill_paid_flag=''";
