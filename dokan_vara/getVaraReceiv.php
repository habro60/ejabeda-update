...
<table class="table bg-light table-bordered table-sm table" id="bill_table">
    ...
    <tbody>
        ...
        $flat_no="SELECT flat_info.flat_no,dokan_vara_detail.flat_no,dokan_vara_detail.donner_pay_amt,dokan_vara_detail.last_paid_date FROM `flat_info`,dokan_vara_detail WHERE `dokan_vara_gl` = '$to_account' and flat_info.flat_no=dokan_vara_detail.flat_no";
        $query2 = mysqli_query($conn, $flat_no);
        $rows2 = mysqli_fetch_assoc($query2);
        $last_flat_no=$rows2['flat_no'];
        $last_paid_date=$rows2['last_paid_date'];
       
        $total_bill = 0;
        $total_b = 0;
        
        // ====== Holiday Sum amount=====
        $sql = "SELECT sum(day_status)  as no_holiday FROM `holiday` WHERE (holiday_date between  '$last_paid_date' and '$bill_date')";
        $query3 = mysqli_query($conn, $sql);
        $rows3 = mysqli_fetch_assoc($query3);
        $no_holiday=$rows3['no_holiday'];
        // =============================
                    $fundsql="SELECT
                    ...
                    DATEDIFF('$bill_date', dokan_vara_detail.last_paid_date)  * dokan_vara_detail.donner_pay_amt - ('$no_holiday' * dokan_vara_detail.donner_pay_amt)  as tran_amt,  date_format(dokan_vara_detail.last_paid_date, '%m/%d/%Y') as ldate
                    FROM
                    dokan_vara_detail,
