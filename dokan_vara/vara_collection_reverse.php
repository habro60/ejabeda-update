...
$conn->query($sql2);
$sqlQuery="select flat_no,paid_date, paid_amount, from_date from vara_receive_detail where `batch_no`='$voucher_no'";
...
$sqlUpdate="update  dokan_vara_detail set `last_paid_date`='$from_date', `donner_paid_amt`=(`donner_paid_amt` -'$paid_amount') where `flat_no`='$flat_no'
";
...
$sql2="DELETE FROM `vara_receive_detail` WHERE `batch_no`= '$voucher_no'";
$conn->query($sql2);
...
if ($conn->affected_rows) {
 ...
 header('refresh:1;../dokan_vara/vara_collection_reverse.php');
}
...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1><i class="fa fa-dashboard"></i>Vara Collection Reverse</h1>
    </div>
  </div>
  <table>
    <form method="POST">
    ...
    <form action="" method="post">
      <table class="table table-hover table-bordered table-sm">
        ...
        <tbody>
          ...
          $mysqli=$conn->query("SET SQL_BIG_SELECTS=1");
        
          $sql = "SELECT tran_details.tran_no, tran_details.tran_date, tran_details.batch_no, tran_details.tran_mode, tran_details.gl_acc_code, tran_details.dr_amt_loc, tran_details.cr_amt_loc, gl_acc_code.acc_head,vara_receive_detail.from_date,vara_receive_detail.paid_date, vara_receive_detail.paid_mode  FROM `tran_details`, gl_acc_code, vara_receive_detail where  tran_details.gl_acc_code=gl_acc_code.acc_code and vara_receive_detail.batch_no=tran_details.batch_no and tran_details.batch_no='$batch_no'";
