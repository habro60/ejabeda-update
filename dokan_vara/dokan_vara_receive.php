...
if (isset($_POST['subBtn'])) {
    ...
    for ($i = 0; $i < count($_POST['flag']); $i++) {
        ...
        if ($flag == 1) {
            $updateQuery = "UPDATE dokan_vara_detail SET `last_paid_date`='$bill_date', `num_of_paid`=(num_of_paid +1), donner_paid_amt=(donner_paid_amt+$donner_pay_amt) WHERE `flat_no`='$flat_no' AND fund_type='$fund_type'";
            // echo $updateQuery;
            // exit; 
    ...
    for ($i = 0; $i < count($_POST['flag']); $i++) {
        ...
        if ($flag == 1) {
            ...
            $insertFund = "INSERT INTO `vara_receive_detail`  (`tran_no`,batch_no,`office_code`,`flat_no`, dokan_vara_gl,paid_mode, `paid_amount`, `paid_date`,from_date, paid_upto,`fund_type`,`fund_type_desc`, `ss_creator`,`ss_creator_on`,`ss_modified_on`,`ss_org_no`) VALUES (NULL,'$batch_no','$office_code','$flat_no', '$to_account','CR','$donner_pay_amt', '$paid_date','$from_date','$paid_upto','$fund_type','$fund_type_desc','$ss_creator',now(),now(),'$ss_org_no')";
    ...
    header('refresh:1;../dokan_vara/dokan_vara_receive.php');
}
...
<main class="app-content">
    <div class="app-title">
        <div>
            <h1><i class="fa fa-dashboard"></i> Dokan Vara Receive</h1>
        </div>
    ...
    <form method="post">
        <table>
            <thead>
                <th>Received Type : </th>
                <th> <a href="../bodri_qafela/fund_receipt_voucher.php"><button type="button" class="btn btn-dark" disabled>Dokan Vara Receive By Cash</button></a></th>
                <th> <a href="../bodri_qafela/fund_receipt_voucher_bank.php"><button type="button" class="btn btn-primary">Dokan Vara Receiveby Cheque</button></a></th>
            </thead>
        </table>
        <hr>
            ...
            <tbody>
                <tr>
                    ...
                    <?php
                        ...
                        ?>
                       <input type="hidden" name="by_account" id="by_account" class="form-control"  value="<?php echo $row['acc_code'] ?>" readonly>
                    </td>
                </tr>
                <thead>
                    <tr>
                        <th>Vara upto Date</th>
                        <th>To Account</th>
                        <th></th>
                        <th></th>
                        <th></th>
                ...
                <tr>
                    ...
                    <td style="width: 200px">
                        <select class="form-control select2" name="to_account" onchange="MyfunctionToAcc()" id="ToAccount" required>
                            ...
                            <?php
                            $selectQuery = "SELECT * FROM `gl_acc_code` where postable_acc= 'Y' AND subsidiary_group_code= '500' ORDER by acc_code";
