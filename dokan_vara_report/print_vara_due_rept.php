...
<main class="app-content">
    ...
    <div class="app-title">
        <div>
            <h1><i class="fa fa-dashboard"></i> Vara Due Report</h1>
        </div>
    </div>
    <!-- end page title -->
    <div class="row">
        <div class="col-md-12">
            <!-- filter option -->
            <form method="POST">
            <?php if (empty($flat_no)) {
    ?>
        <table id="submit">
            <form method="POST">
                <tr>
                    <td>Owner : <select class="form-control grand" name="flat_no">
                            ...
                            if ($selectQueryResult->num_rows) {
                                while ($drrow = $selectQueryResult->fetch_assoc()) {
                                    echo '<option value="' . $drrow['flat_no'] . '">'  . $drrow['flat_title'] . '</option>';
                                }
                            }
                            ?>
                          </select></td>
    ...
    <?php
        ...
        <table id="submit">
            <form method="POST">
                      <tr>
                            <td>
                               <td> <input type="text" name="owner_name" id="" class="form-control" value="<?php echo $row['flat_title']; ?>" readonly></td>
                                <td>As on Date: <input type="date" name="enddate" id="" class="form-control" required></td>
                            <td>
            ...
            ?>
                ...
                <h4 style="text-align:center" id="reportTitle">Vara Due Report</h4>
                <div id="AsOnDate">
                ...
                if (isset($_POST['submit'])) {
                    ...
                    if (empty($flat_no)) {
                            $sql="SELECT dokan_vara_detail.flat_no, dokan_vara_detail.fund_type_desc, dokan_vara_detail.num_of_paid,dokan_vara_detail.pay_amt, flat_info.flat_no, flat_info.flat_title, dokan_vara_detail.donner_paid_amt, DATEDIFF('$enddate', dokan_vara_detail.last_paid_date) AS due_days from dokan_vara_detail, flat_info where  dokan_vara_detail.flat_no=flat_info.flat_no  and dokan_vara_detail.last_paid_date <= '$enddate' and dokan_vara_detail.allow_flag ='1' group by flat_info.flat_no order by flat_info.flat_no"; 

                        }else {

                            $sql="SELECT dokan_vara_detail.flat_no, dokan_vara_detail.fund_type_desc, dokan_vara_detail.num_of_paid,dokan_vara_detail.pay_amt, flat_info.flat_no,flat_info.flat_title, dokan_vara_detail.donner_paid_amt, DATEDIFF('$enddate', dokan_vara_detail.last_paid_date) AS due_days from dokan_vara_detail,flat_info where dokan_vara_detail.last_paid_date <= '$enddate' and dokan_vara_detail.flat_no=flat_info.flat_no  and dokan_vara_detail.flat_no='$flat_no' and dokan_vara_detail.last_paid_date <= '$enddate' and dokan_vara_detail.allow_flag ='1' group by flat_info.flat_no order by flat_info.flat_no";
                ...
                <div id="mySelector">
                    ...
                    <th>Due Amount</th>
                        ...
                        <body>
                            ...
                            ?>
                                <tr>
                     <td style="text-align: right"><?php if ($rows['flat_no'] !=$mem_id) {
                                                           echo $no++; 
                                                       } else {
                                                           echo "";
                                                       }?></td>  
                    <td style="background-color:powderblue; text-align: right"><?php echo $rows['flat_no']; ?></td>
                    <td style="text-align: center"><?php if ($rows['flat_no'] != $mem_id) {
                        ...
                        <td style="background-color:white; text-align: center"><?php echo $rows['due_days']; echo " Days @"; echo $rows['pay_amt']; $due_amt= $rows['pay_amt'] * $rows['due_days']; ?></td>  
                        <td style="background-color:white; text-align: right"><?php echo $due_amt; $tot_due_amt = $tot_due_amt + $due_amt; ?></td>                                                   
