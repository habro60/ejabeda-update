...
if (isset($_POST['subBtn'])) {
  ...
  if ($conn->affected_rows == 1) {
    ...
    $mess = "Failled!";
    header("refresh:1;gl_account_add.php?id=$idno");
  }
    // update 
    $conn->query("UPDATE `dire_info` SET `gl_acc_code`='$acc_code' WHERE dire_name='$acc_head'");
    header('refresh:1;dire_info.php');
}
//select query start
if (isset($_GET['id'])) {
...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1><i class="fa fa-dashboard"></i> Director GL A/C </h1>
    </div>
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <form action="" method="post">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-6">
            <select name="acc_head" id="acc_head" class="form-control" onchange="gl_account_check_availability()" required>
              ...
              <?php
              $selectQuery = 'SELECT * FROM `dire_info`';
              $selectQueryResult =  $conn->query($selectQuery);
              if ($selectQueryResult->num_rows) {
                while ($row2 = $selectQueryResult->fetch_assoc()) {
                  echo '<option value="' . $row2['dire_name'] . '">'  . $row2['dire_name'] . '</option>';
                }
              }
              ?>
            </select>
        ...
        <input type="text" class="form-control" id="" value="8" name="acc_type" hidden>
